 ...
 * 上传角色肖像
 */
export function uploadCharacterPotrait(
  characterId: number,
  body: {
  ...
  >(
    `/p1/wiki/characters/${encodeURIComponent(characterId)}/potraits`,
    oazapfts.json({
      ...opts,
      method: 'POST',
      body,
 ...
 * 上传人物肖像
 */
export function uploadPersonPotrait(
  personId: number,
  body: {
    /** base64 encoded raw bytes, 4mb size limit on **decoded** size */
    img: string;
  ...
  >(
    `/p1/wiki/persons/${encodeURIComponent(personId)}/potraits`,
    oazapfts.json({
      ...opts,
      method: 'POST',
      body,
