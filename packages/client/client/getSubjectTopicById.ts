...
export async function execute(topic_id: number): Promise<Res> {
  const res = await fetch(`/p/subjects/-/topics/${topic_id}`, {
    method: 'get',
 ...
 */
export async function executeX(topic_id: number): Promise<ResX['data']> {
...
export function swrKey(topic_id: number): string {
  return `getSubjectTopicById-${topic_id}`;
}
