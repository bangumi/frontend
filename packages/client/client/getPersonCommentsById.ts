...
export async function execute(person_id: number): Promise<Res> {
 ...
 */
export async function executeX(person_id: number): Promise<ResX['data']> {
...
export function swrKey(person_id: number): string {
  return `getPersonCommentsById-${person_id}`;
}
